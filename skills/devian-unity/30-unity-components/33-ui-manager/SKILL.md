...
```csharp
// Canvas 보장 (없으면 생성)
var hud = UIManager.Instance.EnsureCanvas<MyHudCanvas>("UI/Canvases/MyHudCanvas");

// Canvas 검증
if (!UIManager.Instance.ValidateCanvas<MyHudCanvas>(out var reason))
{
    Debug.LogError($"Canvas validation failed: {reason}");
}

// 커서 설정
UIManager.Instance.SetCursor(true, CursorLockMode.None);

// Canvas 조회만
if (UIManager.Instance.TryGetCanvas<MyHudCanvas>(out var canvas))
{
    // canvas 사용
}

// UI EventSystem 수동 보장 (보통 자동 호출됨)
UIManager.Instance.EnsureUiEventSystem();
```
...
- [UICanvas/UIFrame Overview](../../00-overview/ui-canvas-frames.md)
- [UICanvas/UIFrame SSOT](../../03-ssot/ui-canvas-frames.md)
- [Singleton](../31-singleton/SKILL.md)
- [Pool Factories](../04-pool-factories/SKILL.md)
- [Bootstrap](../27-bootstrap-resource-object/SKILL.md)
