...
- 타입: `BaseTrigger<EntityId, INVENTORY_MESSAGE>`
...
```csharp
public sealed class InventoryMessageTrigger : BaseTrigger<EntityId, INVENTORY_MESSAGE>
{
}
```
...
`INVENTORY_MESSAGE` enum(입력: `input/Domains/Game/ENUM_TYPES.json`)을 사용한다.
...
- `AchieveManager`는 `reqPassId`가 있는 runtime의 WAIT 활성 전이를 위해 Inventory 메시지를 구독한다.
