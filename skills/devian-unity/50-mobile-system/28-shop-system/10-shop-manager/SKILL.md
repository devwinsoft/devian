...
4. 구매 제한 리셋 체크(서버 시간 기준): 구매 제한 상품은 카탈로그별 기준 시각 + 24시간 기준으로 처리한다.
...
11. 구매 제한 상품은 `GetAdsResetRemainingMs(catalogType) == 0`일 때만 성공 시점을 카탈로그 기준 시각(`startedAtUtcMs`)으로 기록
12. `GetAdsResetRemainingMs(catalogType)`으로 카탈로그별 리셋까지 남은 시간(ms)을 조회
13. SaveData 로드 후에는 `RebuildCatalogProductsFromStorage()`로 저장된 daily 상품 리스트(`shopId`, `discountType`, `remainCount`)를 런타임 카탈로그에 복원한다.
14. 로그인 초기화 마지막 단계에서 `LoginManager`가 `Initialize()`를 호출해 Shop 카탈로그 상태를 최종 정합화한다.
15. `SHOP_DAILY` 카탈로그 타이머 만료 시 `dailyCatalogProducts`를 비우고 카탈로그를 rebuild하여 5개 선택 생성을 다시 수행한다.
16. `Initialize()`는 초기 조건(카탈로그 미구성/DAILY 저장 상태 없음) 또는 카탈로그 리셋 조건(24시간 만료)일 때에만 전체 카탈로그 product list를 rebuild한다.
