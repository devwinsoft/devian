...
구매 제한 리셋은 카탈로그별 `autoRefreshUtcMsByCatalog`(다음 refresh 시각) + `autoRefreshDay` 규칙으로 자동 갱신된다.
ADS/FREE 상품 리필은 `adsRefreshUtcMsByCatalog`(다음 ADS/FREE refill 시각)으로 별도 관리한다.
ADS/FREE 구매 성공 시 `adsRefreshUtcMsByCatalog`는 `serverNow + 1day`로 기록된다.
