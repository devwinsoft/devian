# 03-ssot — 45-meta-message-system
...
- 외부 진입점: `MetaMessageManager.Notify(...)` (일부 시스템은 manager helper를 경유 가능)

처리 순서:
1. `MetaMessageManager.Notify(messageType, delta)` 진입
2. `MetaMessageManager`가 `TB_MESSAGE` 기준 `TOTAL_*` 바인딩만 `message.stats[messageId]` 갱신
3. `MetaMessageManager` game trigger publish
