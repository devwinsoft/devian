...
- 효과 구현은 prefab 내부 컴포넌트(ICommonEffectPlayer)로 확장
...
- `com.devian.domain.common`은 `com.devian.foundation`를 의존한다.
...
```
com.devian.domain.common/Editor/Generated/
...
- string wrapper 타입
- `Value` 필드 + `IsValid` 제공
- implicit operator string / COMMON_EFFECT_ID 제공
- GroupKey = `COMMON_EFFECT`
...
- parent는 전달된 `parent`를 그대로 사용
- `parent == null`이면 unparented 상태 유지
...
- `parent`가 명시되면 그 값을 사용
- `parent == null`이면 attachTr를 parent로 사용
...
- Player 자체는 AnimSequenceData나 AnimationClip을 보유하지 않음
