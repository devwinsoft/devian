...
`BaseTrigger<TOwnerKey, TMsgKey>`는 **ownerKey + msgKey 기반 메시지/트리거 라우팅** 시스템이다.
...
```csharp
public class BaseTrigger<TOwnerKey, TMsgKey>
...
{
    private readonly BaseTrigger<int, GameMessage> _ms = new();
...
**BaseTrigger는 시간 기반 이벤트를 지원하지 않는다.**
...
| 타입 | 경로 |
|------|------|
| Runtime | `com.devian.foundation/Samples~/CommonPackage/Runtime/Unity/BaseTrigger/BaseTrigger.cs` |

---

## Reference

- 인덱스: `20-common-package/00-overview/SKILL.md`
