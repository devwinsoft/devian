...
프리팹 생성 규칙은 [11-mobile-application](../../50-mobile-package/11-mobile-application/SKILL.md)에서 다룬다.

---

## 2. Files (SSOT)

- `framework-cs/upm/com.devian.foundation/Samples~/CommonPackage/Runtime/Unity/Application/BaseApplication.cs`
...
2. `_loaded = true`
3. `await onLoadAsync(onProgress)` — patch/download + bundle 로드
4. `await onLoadCompletedAsync()` — 매니저 초기화

의미:
- `BootProc()`와 같은 idempotent 패턴 (1회 실행, 재시도 없음)
- 앱 리소스 로딩을 위한 템플릿
- 언어 설정은 `MobileApplication.DefaultLanguage`가 owner이며, 파생 클래스가 직접 참조한다
- 번들 에셋 로드는 `BundleManager<T>.LoadBundlesAsync()` → `onLoadBundlesAsync()` 체인이 담당한다
...
- Parent: `skills/devian-unity/20-common-package/00-overview/SKILL.md`
- SceneTransManager: `skills/devian-unity/20-common-package/28-scene-trans-manager/SKILL.md`
- Singleton: `skills/devian-unity/20-common-package/29-singleton/SKILL.md`
