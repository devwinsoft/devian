...
| 파일 | 설명 |
|------|------|
| `Runtime/Generated/Common.g.cs` | TableGen으로 생성된 Common 모듈 코드 (CommonErrorType enum 포함) |
| `Runtime/Module/CoreError.cs` | 에러 정보 컨테이너. 공식 네임스페이스: `Devian.Domain.Common` |
| `Runtime/Module/CoreResult.cs` | 성공/실패 결과 타입. 공식 네임스페이스: `Devian.Domain.Common` |
...
> 소스 SSOT: `framework-cs/module/Devian.Domain.Common/src/`
...
CoreResult<T>.Failure(CommonErrorType errorType, string message)
...
```csharp
// 주 사용
new CoreError(CommonErrorType errorType, string message, string? details = null)
