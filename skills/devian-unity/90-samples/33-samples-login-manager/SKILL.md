# 33-samples-login-manager — Login Manager


## Singleton
- `LoginManager`는 `CompoSingleton<LoginManager>` 기반이다.
...
- Google Play Games Services (GPGS) Unity 플러그인: **Play Games Plugin for Unity v2.0.0+** (권장)
  - LoginManager는 GPGS 플러그인을 **Reflection**으로 호출한다.
  - v2 기준 흐름을 문서화하며, 필요 시 구버전(v1 계열) 시그니처도 호환 경로로 처리한다.


## Login Type (Platform)
- **Editor Login**: Firebase Anonymous
- **Guest Login (device)**: Firebase Anonymous — CloudSaveManager를 Initialize/Load/Save 호출하지 않음 (로컬 저장만 사용). Sync 호출이 있어도 Cloud 접근 없이 no-op.
...
3. `CloudSaveManager.Instance.InitializeAsync(ct)` — Guest 로그인에서는 스킵

Sync는 LoginManager의 책임이 아니며, [37-samples-syncdata-manager](../37-samples-syncdata-manager/SKILL.md)가 담당한다.
...
- UPM: `framework-cs/upm/com.devian.samples/Samples~/MobileSystem/Runtime/Login/LoginManager.cs`
- UnityExample mirror: `framework-cs/apps/UnityExample/Packages/com.devian.samples/Samples~/MobileSystem/Runtime/Login/LoginManager.cs`
...
- Sync 오케스트레이션 — SyncDataManager가 담당
