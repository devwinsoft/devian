...
- `ApplyDeltasAsync(deltas, ct)`
  - `InventoryDelta[]`를 로컬 인벤토리에 적용한다.
- `(선택) ApplyRewardIdAsync(rewardId, ct)`
  - rewardId를 **컨텐츠 레이어**에서 InventoryDelta로 해석한 뒤 적용한다.
  - rewardId 정본은 Game 도메인 테이블 `REWARD`의 `rewardId(pk)`다: `input/Domains/Game/RewardTable.xlsx`
...
- Purchase: (NEEDS CHECK) 서버 검증 결과가 지급 확정일 때만 `InventoryDelta[]`(또는 rewardId 목록)를 RewardManager로 적용한다.
- Mission: MissionManager가 로컬 ledger 기준으로 "지급 결정/중복 방지" 후 RewardManager에 `rewardId` 지급 실행을 위임한다.
