...
```
framework-cs/upm/com.devian.foundation/Samples~/UIPackage/Runtime/UIUtils.cs
```
...
| 메서드 | 이전 위치 | 비고 |
|--------|-----------|------|
| `TryWorldToOverlayLocal` | `UICanvas<T>` | `canvas.worldCamera` → 파라미터 `worldCamera` |
| `ComputeBillboardRotation` | `UICanvas<T>` | `canvas.worldCamera` → 파라미터 `camera` |
| `ApplyBillboard` | `UICanvas<T>` | `canvas.worldCamera` → 파라미터 `camera` |
...
| Dependency | Location |
|------------|----------|
| `BillboardMode` | `UICanvas.cs` (같은 Runtime 폴더) |
