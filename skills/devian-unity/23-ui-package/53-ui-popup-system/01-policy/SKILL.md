...
- popup open은 `PopupId` string이 아니라 frame type 기반 generic API로 처리한다.
- top popup만 입력 가능하다.
...
- open / close preset 의미는 `UIPopupFrameBase` 또는 concrete popup frame이 가진다.
- `UIPopupFrameBase`는 `Opening / Opened / Closing` 상태를 가진다.
