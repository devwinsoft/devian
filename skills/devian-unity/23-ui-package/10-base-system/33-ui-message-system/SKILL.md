...
```
framework-cs/upm/com.devian.foundation/Samples~/UIPackage/Runtime/Base/UIMessageSystem.cs
```
...
|-------|---------|
| `None` | 기본값 |
| `InitOnce` | `UIBaseCanvas.Init()` 완료 후 1회 발행 |
| `ReloadText` | 텍스트 리로드 요청 |
| `Resize` | UI 리사이즈 통지 |

### Notify Timing

`UIBaseCanvas.Init()` 마지막 단계에서 `UIManager.messageSystem.Notify(UI_MESSAGE.InitOnce)`가 호출된다.
...
| `UIManager` | `framework-cs/upm/com.devian.foundation/Samples~/UIPackage/Runtime/Base/UIManager.cs` |
