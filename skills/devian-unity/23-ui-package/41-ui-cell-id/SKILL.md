...
UIGridCell 프리팹을 참조하기 위한 string wrapper ID 타입.

## 파일 위치 (SSOT)

- Runtime: `com.devian.foundation/Samples~/UIPackage/Runtime/Container/UI_CELL_ID.cs`
- Editor Selector: `com.devian.foundation/Samples~/UIPackage/Editor/UICellIdSelector.cs`
- Editor Drawer: `com.devian.foundation/Samples~/UIPackage/Editor/UI_CELL_ID_Drawer.cs`
...
### SearchDir 공급
- BundleSettings.GetEntry(`"UI_CELL_ID"`)로 조회
- 실패/폴더 없음이면 `"Assets"` fallback

### 스캔 대상
- UIGridCell 컴포넌트가 있는 Prefab 목록을 SearchDir에서 스캔
- `BaseEditorAssetIdSelector<UIGridCell>` 재활용 (AssetManager.FindPrefabs)
...
- UIGridCell: `skills/devian-unity/23-ui-package/23-ui-frame-grid/SKILL.md`
