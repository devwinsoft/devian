...
`UICanvas` 프리팹을 참조하기 위한 string wrapper ID 타입.

## 파일 위치 (SSOT)

- Runtime: `com.devian.foundation/Samples~/UIPackage/Runtime/UI_CANVAS_ID.cs`
...
`UI_CONTAINER_ID`, `UI_CELL_ID`와 동일한 구조:
...
- `UICanvas` 또는 concrete subclass가 있는 Prefab 목록을 SearchDir에서 스캔
- `BaseEditorAssetIdSelector<UICanvas>` 재활용 (`AssetManager.FindPrefabs`)
...
- `UICanvas`는 abstract base지만, selector는 prefab root의 `GetComponent<UICanvas>()`로 concrete subclass를 잡는다
...
- UICanvas System: `skills/devian-unity/23-ui-package/11-ui-canvas-system/SKILL.md`
