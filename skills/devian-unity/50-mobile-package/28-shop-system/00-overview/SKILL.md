...
`SHOP_ITEM_DAILY`는 초기화/refresh 시 `select_rate` 규칙으로 ADS/FREE 제외 대상에서 5개 상품을 선택 생성한다.
`CHEST/PURCHASE/GOLD`는 각 `SHOP_*` 테이블의 모든 상품을 생성한다.
`EVENT`는 `SHOP_ITEM_EVENT.start_time/end_time` 서버 UTC 시간 구간 안에 있는 상품만 생성한다.
`SHOP_ITEM_DAILY`의 ADS/FREE 상품은 고정 상품으로 항상 테이블에서 로드되며, 5개 선택 대상/저장 대상에 포함되지 않는다.
