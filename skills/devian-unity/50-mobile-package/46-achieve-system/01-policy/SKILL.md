...
- 초기화 시 `achieve_id` group 기준 runtime을 항상 생성한다(1 group = 1 runtime).
...
- `SOCIAL` row는 `req_msg_id/req_value`가 있으면 `WAIT` 상태로 시작한다.
- `PASS` row는 `req_pass_id` 또는 `req_season_id`가 있으면 `WAIT` 상태로 시작한다.
- `WAIT` 상태에서는 `condition_msg_id` 진행도 반영을 하지 않고, req 조건 충족 시 `ACTIVE`로 전이한다.

### 2) 외부에는 내부 업적 ID만 노출한다

- 공개 API는 `achievementId`만 받는다.
- 플랫폼 ID(`apple_achievement_id`, `google_achievement_id`)는 `ACHIEVE_SOCIAL` 매핑 레이어 내부에만 존재한다.
...
- 다음 level row 기준으로 `condition_msg_id/messageType/saveType/condition_value`를 교체한다.
- level-up 시에도 타입별 req 조건을 다시 평가하여 `WAIT` 또는 `ACTIVE`로 시작한다.
  - `SOCIAL`: `req_msg_id/req_value`
