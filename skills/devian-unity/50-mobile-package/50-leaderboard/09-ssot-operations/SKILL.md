...
- `LeaderboardManager.ReportScoreAsync(leaderboard_id, ct)` 호출
- score는 `LEADERBOARD.message_id` 기준으로 내부 조회한다.
- 시즌 활성 기간 외에는 점수 기록 실패 반환 (`LEADERBOARD.season_id → TB_SEASON` 시간 확인)
...
- `Success + HasScore=true`:
  ...
  - 매칭 성공: `RewardManager.ApplyRewardGroup(reward_group_id)` + `CLAIMED` 기록
...
- grace period 경계:
  - 시즌 종료 + 9분: 미평가
  - 시즌 종료 + 10분: 평가 시작
- `processedClaims` 중복 지급 방지(같은 `leaderboard_id` 1회)
...
- 하드코딩 보상 지급 0건 (`reward_group_id` 경유만 허용)
- 시즌 외 점수 기록 차단 정상 동작
- 공개 경계 플랫폼 의존성 노출 0건
