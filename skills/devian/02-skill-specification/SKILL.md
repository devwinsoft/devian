...
코드/생성물/시그니처/프레임 포맷 같은 구현 세부는 **`docs/generated/devian-reference.md`**를 참조한다.
...
- build.json 기준으로 staging/final 경로를 어떻게 해석하는지
- 플레이스홀더 표준
...
아래는 **SKILL에서 금지**한다(코드 기반 Reference로 이동).
...
- 용어: DomainType / DomainKey / LinkName / ProtocolName
- 플레이스홀더: `{tempDir}`, `{DomainKey}`, `{LinkName}`, `{ProtocolName}`, `{csTargetDir}`, `{tsTargetDir}`, `{dataTargetDir}`, `{upmTargetDir}`
