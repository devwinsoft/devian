...
## 의존 방향 규칙 (Dependency Direction)

### 기반 스킬은 소비자 스킬을 언급하지 않는다

- **기반 스킬**(사용 당하는 스킬)은 자신을 사용하는 **소비자 스킬**(사용하는 스킬)의 이름·링크·도메인 상세를 포함하지 않는다.
- 의존 참조는 **소비자 → 기반** 방향만 허용한다.
- 기반 스킬이 "자신의 비책임"을 명시할 때는 소비자 이름 대신 **일반 용어**("호출자", "상위 시스템")를 사용한다.

예:
- Good: "멱등/기록/복구는 호출자가 책임진다."
- Bad: "멱등/기록/복구는 MissionManager / PurchaseManager가 책임진다."

이유:
- 소비자가 추가/변경되어도 기반 스킬 문서를 수정할 필요가 없다.
- 기반 스킬의 독립성과 재사용성을 보장한다.
