...
구체적인 타입/시그니처/호출 예시는 **`docs/generated/devian-reference.md`**를 정답으로 본다.
...
### 2) PROTOCOL 산출물 (LinkName/ProtocolName 단위)
...
1) `{dataTargetDir}/json/*.ndjson`를 로드한다.
...
- build.json의 targetDirs 충돌을 피하도록 설계한다(클린-카피로 인한 덮어쓰기 방지)
...
- Transport: `skills/devian/40-ws-transport-adapter/SKILL.md`
