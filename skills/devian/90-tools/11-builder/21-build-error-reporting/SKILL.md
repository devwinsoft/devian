...
- `<LOC>`는 기본적으로 `<file><jsonPath>` 형태
  - 예: `devian/{buildInputJson}$.domains[1].name` (예: `devian/input/build_input.json$.domains[1].name`)
- `<MESSAGE>`는 1줄, **줄바꿈 금지**
...
| `file` | string | 입력 파일 경로 (레포 기준 상대경로) | `"devian/{buildInputJson}"` (예: `"devian/input/build_input.json"`) |
...
- `skills/devian/90-tools/11-builder/21-build-error-reporting/SKILL.md`

### 빌드 관련

- `skills/devian/90-tools/11-builder/20-build-domain/SKILL.md`
