...
1) 테이블 데이터를 로드한다:
   - NDJSON: `{tableDir}/ndjson/*.json` (내용은 NDJSON)
...
- `{buildInputJson}`의 targetDirs 충돌을 피하도록 설계한다(클린-카피로 인한 덮어쓰기 방지)
...
- Transport: `skills/devian/10-module/70-ws-transport-adapter/SKILL.md`
- 동작 정본: 런타임/제너레이터 코드
