...
- `{buildInputJson}` (예: `input/build_input.json`)에서 `domains.Common`은 필수 항목이다.
- `configPath`는 `{projectConfigJson}` (예: `input/build_config.json`)을 가리켜야 한다.
...
- C# Domain: `.csproj`에 `Devian.csproj` ProjectReference 포함
- C# Protocol: `.csproj`에 `Devian.csproj` + `Devian.Domain.Common.csproj` ProjectReference 포함
- C# 생성물(`*.g.cs`): `using Devian;` 포함 (namespace는 Devian 단일)
- TS Domain: `package.json` `dependencies`에 `@devian/core` 포함
- TS Protocol: `package.json` `dependencies`에 `@devian/core` + `@devian/module-common` 포함
...
- C#: `Devian.Domain.Common/features/**`
...
> 프로젝트 이름(`Devian.Domain.Common`)과 namespace(`Devian`)는 별개다.
...
- Domain Policy: `skills/devian/20-domain-common/01-policy/SKILL.md`
- Core Features: `skills/devian/10-module/20-core/00-overview/SKILL.md`
