...
- **입력:** `{buildInputJson}` (예: `input/build_input.json`)
- **설정:** `{configJson}` (예: `input/build_config.json`)

### 비대상

- 런타임 실행 로직
- 에러/로그 표준 (→ `skills/devian/80-tools/11-builder/21-build-error-reporting/SKILL.md`)
...
| 위치 | Forbidden 필드 |
|------|-----------------|
| `{configJson}` | `dataConfig` (deprecated) |
| `domains[*]` | `csTargetDir` |
| `domains[*]` | `tsTargetDir` |
...
예제 입력 위치:
- `devian/input/Domains/Game/*.json`
- `devian/input/Domains/Game/*.xlsx`
...
- `input/build_input.json` — 빌드 입력 예시
- `input/build_config.json` — 설정 파일 예시

### 관련 스킬

| 스킬 | 설명 |
|------|------|
| `skills/devian/10-module/03-ssot/SKILL.md` | 전체 SSOT 정책 |
| `skills/devian/80-tools/11-builder/21-build-error-reporting/SKILL.md` | 빌드 에러/로그 표준 |
