# Devian v10 — Common Domain Policy
...
> **Note**: Common 모듈 전체 정책(생성물/수동 코드 경계, features 구조 등)은 `01-module-policy`를 참조한다.
...
1) Common도 다른 DomainKey와 동일한 방식으로 build.json에 정의한다.
...
- final: build.json의 `domains.Common.*TargetDirs`가 정본
