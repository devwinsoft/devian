...
function generateProxy(lines, messages, protocolName, usesComplexAliases = false) {
    ...
    lines.push('            private bool _connectRejectNotified; // Connect-reject dedup guard');
    lines.push('');
    lines.push('            // ======== Codec ========');
    ...
    lines.push('            /// Connect is allowed only when state is Disconnected.');
    lines.push('            /// Any attempt in other states fails immediately (OnError invoked).');
    ...
    lines.push('');
    lines.push('                // Fail if already connecting OR state exists and is not Disconnected.');
    lines.push('                if (_isConnecting || (state.HasValue && state.Value != NetClientState.Disconnected))');
    lines.push('                {');
    lines.push('                    // Dedup: only notify once until state changes/reset');
    lines.push('                    if (_connectRejectNotified)');
    lines.push('                        return;');
    lines.push('');
    lines.push('                    _connectRejectNotified = true;');
    lines.push('');
    lines.push('                    var msg =');
    lines.push('                        "Connect() is only allowed when state is Disconnected. Current state=" +');
    lines.push('                        (state.HasValue ? state.Value.ToString() : "null") + ".";');
    lines.push('');
    lines.push('                    var ex = new InvalidOperationException(msg);');
    lines.push('                    _lastError = msg;');
    lines.push('');
    lines.push('                    OnError?.Invoke(ex);');
    lines.push('                    return;');
    lines.push('                }');
    lines.push('');
    lines.push('                // If we have a disconnected session but URL differs, dispose before new attempt.');
    lines.push('                if (_session != null && !string.Equals(_url, url, StringComparison.Ordinal))');
    lines.push('                    DisposeConnection();');
    lines.push('');
    lines.push('                _url = url;');
    lines.push('                _lastError = string.Empty;');
    lines.push('                _connectRejectNotified = false;');
