...
/**
 * Generate thin C# ClientSessionHost for a protocol group.
 * Lifecycle lives in foundation NetClientSessionHost; generated code supplies pairing only.
 * @param {string} groupName
 * @param {Array} protocolInfos - Array of { name, direction }
 * @returns {string|null}
 */
export function generateCSharpClientSessionHost(groupName, protocolInfos) {
    ...
    lines.push('{');
    lines.push('    /// <summary>');
    lines.push(`    /// Client session host for the ${groupName} protocol group.`);
    lines.push('    /// Supplies protocol pairing to the shared NetClientSessionHost lifecycle.');
    lines.push('    /// </summary>');
    lines.push('    public sealed class ClientSessionHost : NetClientSessionHost');
    lines.push('    {');

    if (inboundProtos.length > 1) {
    ...
    const stubParams = inboundProtos.map(p => `${p.name}.Stub ${lowerFirst(p.name)}Stub`);
    const proxyParams = outboundProtos.map(p => `${p.name}.Proxy ${lowerFirst(p.name)}Proxy`);
    const ctorParams = [...stubParams, ...proxyParams, 'INetConnector? connector = null'];

    lines.push('        public ClientSessionHost(');
    lines.push(`            ${ctorParams.join(',\n            ')}`);
    lines.push('        )');
    lines.push('            : base(');

    if (inboundProtos.length === 1) {
        const proto = inboundProtos[0];
        lines.push(`                () => new ${proto.name}.Runtime(${lowerFirst(proto.name)}Stub),`);
    } else {
        lines.push('                () => new RuntimeMux(');
        const runtimeArgs = inboundProtos.map(p => `new ${p.name}.Runtime(${lowerFirst(p.name)}Stub)`);
        lines.push(`                    ${runtimeArgs.join(',\n                    ')}`);
        lines.push('                ),');
    }

    const bindingArgs = outboundProtos.map(p => `${lowerFirst(p.name)}Proxy`);
    lines.push(`                new INetSessionBindable[] { ${bindingArgs.join(', ')} },`);
