...
`;
     ...
     * - Runtime/_Shared/UnityMainThreadDispatcher.cs (main thread dispatcher)
     */
...
`;
        ...
        // Generate UnityMainThreadDispatcher.cs (main thread dispatcher for logging)
        const unityMainThreadDispatcherCode = `// <auto-generated>
...
`;
        fs.writeFileSync(path.join(outDir, 'UnityMainThreadDispatcher.cs'), unityMainThreadDispatcherCode, 'utf-8');
        
        console.log(`    [OK] Generated unity shared: Runtime/_Shared (2 files)`);
    }
...
`;
    ...
    /**
     * Generate Unity Pool Factories implementation files.
     * Provides concrete IPoolFactory implementations.
     * SSOT: skills/devian-upm/30-unity-components/04-pool-factories/SKILL.md
     * 
     * Generates:
     * - Runtime/PoolFactories/InspectorPoolFactory.cs
     * - Runtime/PoolFactories/BundlePoolFactory.cs
     */
    generateUnityPoolFactoriesRuntime(stagingUpm) {
        const outDir = path.join(stagingUpm, 'Runtime', 'PoolFactories');
        
        // Clean and create output directory (deterministic)
        fs.rmSync(outDir, { recursive: true, force: true });
        fs.mkdirSync(outDir, { recursive: true });
        
        // Generate InspectorPoolFactory.cs
        const inspectorPoolFactoryCode = `// <auto-generated>
...
`;
        fs.writeFileSync(path.join(outDir, 'InspectorPoolFactory.cs'), inspectorPoolFactoryCode, 'utf-8');
        
        // Generate BundlePoolFactory.cs
        const bundlePoolFactoryCode = `// <auto-generated>
...
`;
        fs.writeFileSync(path.join(outDir, 'BundlePoolFactory.cs'), bundlePoolFactoryCode, 'utf-8');
        
        console.log(`    [OK] Generated unity pool factories: Runtime/PoolFactories (2 files)`);
    }
