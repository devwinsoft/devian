...
import { generateTypeScriptProtocol, generateServerRuntime } from './generators/protocol-ts.js';
...
class DevianToolBuilder {
    ...
    async processProtocolGroup(groupConfig) {
        ...
        // Generate index.ts for TypeScript
        const indexTsContent = this.generateIndexTs(protocolNames, hasServerRuntime);
...
`;
    ...
    generateIndexTs(protocolNames, hasServerRuntime = false) {
        const exports = protocolNames.map(name => `export * from './${name}.g';`);
        if (hasServerRuntime) {
            exports.push(`export * from './generated/ServerRuntime.g';`);
        }
        return exports.join('\n') + '\n';
    }
    ...
    async copyProtocolGroupToTargets(groupConfig) {
        ...
        // Check if ServerRuntime was generated
        const hasServerRuntime = fs.existsSync(path.join(stagingTs, 'generated', 'ServerRuntime.g.ts'));

        // Copy to CS target: {csTargetDir}/Devian.Protocol.{ProtocolGroup}/
        ...
        // Copy to TS target: {tsTargetDir}/devian-protocol-{group}/
        if (groupConfig.tsTargetDir) {
            ...
            this.ensureProtocolPackageJson(target, groupName, hasServerRuntime);
        }
...
`;
    ...
    ensureProtocolPackageJson(targetDir, groupName, hasServerRuntime = false) {
        const packageJsonPath = path.join(targetDir, 'package.json');
        ...
        if (hasServerRuntime) {
            dependencies['@devian/network-server'] = '10.0.0';
        }
