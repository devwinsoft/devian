 ...
 * with the Sample protocol group's server runtime.
 * 
 * This file only:
...
async function main() {
    ...
    const transport = new WsTransport(
        ...
        {
            onConnect: (sessionId) => {
                console.log(`[SampleServer] Session ${sessionId} connected`);
            },
            onDisconnect: (sessionId) => {
                console.log(`[SampleServer] Session ${sessionId} disconnected`);
            },
            ...
            onError: (sessionId, error) => {
                console.error(`[SampleServer] Session ${sessionId} error:`, error.message);
            },
        }
    );
    ...
    server = new NetworkServer(transport, runtime, {
        onUnknownInboundOpcode: async (event: UnknownOpcodeEvent) => {
            ...
            console.warn(
                `[SampleServer] Unknown opcode ${event.opcode} from session ${event.sessionId}`,
    ...
    const sample2cProxy = server.createOutboundProxy<Sample2C.Proxy>();
    ...
    stub.onPing(async (sessionId, msg) => {
        ...
        // Send Pong response
        await sample2cProxy.sendPong(sessionId, {
    ...
    stub.onEcho(async (sessionId, msg) => {
        ...
        await sample2cProxy.sendEchoReply(sessionId, {
            message: msg.message,
    ...
    console.log(`[SampleServer] Running on port ${PORT}`);

    // Graceful shutdown
    process.on('SIGINT', async () => {
        console.log('\n[SampleServer] Shutting down...');
