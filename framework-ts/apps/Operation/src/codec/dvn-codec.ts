 ...
 */
export async function encodeDvn(json: string): Promise<string> {
 ...
 */
export async function decodeDvn(dvnContent: string): Promise<string> {
...
// ──────────────────────────────────────
function decodeDvnV1(dvnContent: string): string {
...
// ──────────────────────────────────────
async function decodeDvnV2(dvnContent: string): Promise<string> {
  ...
  const separatorIndex =
    signedPayload.length - HMAC_HEX_LENGTH - HMAC_SEPARATOR.length;
  const separator = signedPayload.substring(
    separatorIndex,
    separatorIndex + HMAC_SEPARATOR.length,
  );
  ...
  const hmacHex = signedPayload.substring(
    separatorIndex + HMAC_SEPARATOR.length,
  );
...
function extractSocialUserId(json: string): string | null {
  try {
