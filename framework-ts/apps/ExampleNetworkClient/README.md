...
ExampleNetworkServer와의 왕복 통신을 통해 Proxy/Stub + defaultCodec + frame 포맷을 검증합니다.
...
```bash
cd framework-ts/apps/ExampleNetworkServer
npm install
npm start
```
...
```bash
cd framework-ts/apps/ExampleNetworkClient
npm install
npm run dev
```
