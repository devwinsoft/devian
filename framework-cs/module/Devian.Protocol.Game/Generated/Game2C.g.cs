    ...
    public static partial class Game2C
    {
        ...
        public sealed class CodecProtobuf : ICodec
        {
            ...
            public void EncodeTo<T>(IBufferWriter<byte> writer, T message) where T : class
            {
                switch (message)
                {
                    case Pong m: EncodePong(writer, m); break;
                    case EchoReply m: EncodeEchoReply(writer, m); break;
            ...
            private static void EncodePong(IBufferWriter<byte> writer, Pong m)
            {
                ProtoWriter.WriteInt64(writer, 1, m.Timestamp);
                ProtoWriter.WriteInt64(writer, 2, m.ServerTime);
            }

            private static void EncodeEchoReply(IBufferWriter<byte> writer, EchoReply m)
            {
                ProtoWriter.WriteString(writer, 1, m.Message);
                ProtoWriter.WriteInt64(writer, 2, m.EchoedAt);
            }
        ...
        public sealed class Proxy : IDisposable
        {
            ...
            public void SendPong(Pong message)
            {
                ...
                BitConverter.TryWriteBytes(span, Opcodes.Pong);
                bw.Advance(4);
            ...
            public void SendEchoReply(EchoReply message)
            {
                ...
                // Write opcode (4 bytes LE)
                var span = bw.GetSpan(4);
                BitConverter.TryWriteBytes(span, Opcodes.EchoReply);
                bw.Advance(4);
