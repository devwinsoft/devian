...
namespace Devian.Network.Transports
{
    ...
    /// Routes events to the appropriate WebSocketClient instance.
    /// </summary>
    internal sealed class WebGLWsDriver : MonoBehaviour
    {
        ...
        private readonly Dictionary<int, WebSocketClient> _clients = new();
        ...
        /// </summary>
        public void Register(int socketId, WebSocketClient client)
        {
            lock (_lock)
            {
        ...
        private WebSocketClient? GetClient(int socketId)
        {
            lock (_lock)
            {
