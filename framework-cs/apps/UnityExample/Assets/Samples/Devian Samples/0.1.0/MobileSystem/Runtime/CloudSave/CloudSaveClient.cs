...
namespace Devian
{
    ...
    /// </summary>
    public sealed class CloudSaveClient : CloudSaveClientApple
    {
        ...
        public async Task<CloudSaveResult> SignInIfNeededAsync(CancellationToken ct)
        {
            ...
            {
                UnityEngine.Debug.LogError($"[CloudSaveClient] init failed: {init}");
                return init;
            }
            ...
            UnityEngine.Debug.LogError("[CloudSaveClient] AuthRequired: FirebaseAuth.CurrentUser is null. Login flow must sign in before CloudSave.");
            ...
            string slot, CancellationToken ct)
        {
            ...
            {
                UnityEngine.Debug.LogError($"[CloudSaveClient] LoadAsync failed. slot='{slot}' ex={ex}");
        ...
        private async Task<CloudSaveResult> ensureInitializedAsync(CancellationToken ct)
        {
            ...
            {
                _initialized = false;
                UnityEngine.Debug.LogError($"[CloudSaveClient] ensureInitializedAsync exception: {ex}");
                return CloudSaveResult.NotAvailable;
            }
        }
