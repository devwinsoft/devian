...
namespace Devian
{
    ...
    public sealed class AccountLoginGpgs
    {
        ...
        private void ensureReflection()
        {
            ...
            {
                Debug.LogWarning($"[AccountLoginGpgs] Reflection init failed: {e.Message}");
                _isAvailable = false;
            }
#else
            _isAvailable = false;
...
#if UNITY_ANDROID && !UNITY_EDITOR
            ensureReflection();
            if (!_isAvailable) return Task.FromResult(SaveCloudResult.NotAvailable);
        ...
        public async Task<CommonResult<LoginCredential>> GetServerAuthCodeCredentialAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (req == null)
                {
                    return CommonResult<LoginCredential>.Failure(CommonErrorType.LOGIN_GPGS_NO_SERVER_ACCESS,
                ...
                {
                    return CommonResult<LoginCredential>.Failure(CommonErrorType.LOGIN_GPGS_NO_AUTH_CODE,
        ...
        public async Task<SaveCloudResult> DeleteAsync(string slot, CancellationToken ct)
        {
            ...
            try
            {
                ...
                return isSuccess(stObj) ? SaveCloudResult.Success : mapStatus(stObj);
            }
            catch
            {
                return SaveCloudResult.FatalFailure;
