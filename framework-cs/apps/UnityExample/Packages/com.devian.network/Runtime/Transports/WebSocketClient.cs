...
namespace Devian.Network.Transports
{
    ...
    {
        private readonly NetworkClient _core;
        ...
        public void Connect(string url, string[]? subProtocols = null)
        {
            ...
            // Start send thread
            _sendThread = new Thread(SendLoop) { IsBackground = true, Name = "WebSocketClient.Send" };
            _sendThread.Start();

            // Start recv thread
            _recvThread = new Thread(RecvLoop) { IsBackground = true, Name = "WebSocketClient.Recv" };
            _recvThread.Start();
